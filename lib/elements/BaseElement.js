...
class BaseElement {
     ...
     */
    constructor(locator, timeout = 60000) {
     ...
     */
    async clickWhenClickable(index = 0, timeout = this.defaultTimeout) {
        await driver.wait(() =>
            {
                return this.click(index)
                    .then(() => true, () => false)
            }, timeout, `clickWhenClickable timed out after ${timeout} ms. ${this.locatorString}`
        )};
